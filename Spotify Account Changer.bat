...
title %title% - List
cls
dir /b /a-d %spotify%\Users|find /v "." >nul 2>&1
if errorlevel 1 (
...
copy %spotify%\prefs %spotify%\Users\%accountname% >nul 2>&1
echo.
...
if not defined option goto :invalid
if %option% == y (
	del /q %spotify%\Users\%newname% >nul 2>&1
	ren %spotify%\Users\%account% %newname% >nul 2>&1
	echo.
	echo Account "%account%" has been renamed to "%newname%"!
	goto :back
...
taskkill /f /im Spotify.exe /t >nul 2>&1
del /q %spotify%\prefs 2>nul 2>&1
copy %spotify%\Users\%account% %spotify% >nul 2>&1
ren %spotify%\%account% prefs >nul 2>&1
C:
cd %soggfy%
start "" Injector.exe
echo.
